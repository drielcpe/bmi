...
if __name__ == '__main__':
    try:
        ...
        hx = HX711(dout_pin=21, pd_sck_pin=20, gain_channel_A=128, select_channel='A')

        err = hx.reset()  # Reset the HX711
        if err:
        ...
        # Set scale ratio (calibration)
        input('Place a known weight on the scale and press Enter to calibrate...')
        known_weight = float(input('Enter the weight in grams: '))
        raw_data = hx.get_raw_data_mean(readings=30)
        scale_ratio = raw_data / known_weight
        hx.set_scale_ratio(scale_ratio)
        print('Scale ratio set to:', scale_ratio)
